...
solverOutput leona_knn2(const std::string& folder_path,
    ...
    std::string sto_path = folder_path + "/sto.txt";
    std::string resultsOutput_path = folder_path + "/computationalResults(leona_knn1.0).txt";
    std::string resultsSolution_path = folder_path + "/sol(leona_knn1.0).txt"; // ***
    // convert all the paths into constant chars
    const char* be_DB_path_const = be_DB_path.c_str();
    const char* bi_DB_path_const = bi_DB_path.c_str();
    const char* Ce_DB_path_const = Ce_DB_path.c_str();
    ...
    for (int outer_it = 0; outer_it < max_it; ++outer_it) {
        ...
        // write a sequence of solutions iteration, N, time, solution
        if (idx_pointer < it_pointer.size() && outer_it == it_pointer[idx_pointer] - 1) {
            double duration2 = (std::clock() - time_start ) / (double) CLOCKS_PER_SEC;
            writeFileSol << it_pointer[idx_pointer] << "," << N_total << "," << duration2;
            for (int idx_x = 0; idx_x < x_leon.size(); ++idx_x) {
                writeFileSol << "," << x_leon[idx_x];
            }
...
solverOutput leona_kernel2(const std::string& folder_path,
    ...
    std::string sto_path = folder_path + "/sto.txt";
    std::string resultsOutput_path = folder_path + "/computationalResults(leona_kernel1.0).txt";
    std::string resultsSolution_path; // ***
    // convert all the paths into constant chars
    const char* be_DB_path_const = be_DB_path.c_str();
    const char* bi_DB_path_const = bi_DB_path.c_str();
    const char* Ce_DB_path_const = Ce_DB_path.c_str();
    ...
    switch (flag_kernel) {
        case 1:
            std::cout << "Naive kernel is used\n";
            writeFile << "Naive kernel is used\n";
            resultsSolution_path = folder_path + "/sol(leona_N1.0).txt";
            break;
        case 2:
            std::cout << "Epanechnikov kernel is used.\n";
            writeFile << "Epanechnikov kernel is used.\n";
            resultsSolution_path = folder_path + "/sol(leona_E1.0).txt";
            break;
        case 3:
            std::cout << "Quartic kernel is used.\n";
            writeFile << "Quartic kernel is used.\n";
            resultsSolution_path = folder_path + "/sol(leona_Q1.0).txt";
            break;
        case 4:
            std::cout << "Gaussian kernel is used.\n";
            writeFile << "Gaussian kernel is used.\n";
            resultsSolution_path = folder_path + "/sol(leona_G1.0).txt";
            break;
        default:
            throw std::logic_error("Kernel flag is incorrect! Solver is terminated.\n");
            break;
